...
function optionsFrom(arguments_) {
  const options = { port: DEFAULT_PORT, enabled: false, testMode: false, testPort: null, controlSocket: null,
    httpIdleTimeoutMs: HTTP_IDLE_TIMEOUT_MS, testIdleOverride: false };
  for (let i = 0; i < arguments_.length; i += 1) {
    switch (arguments_[i]) {
      ...
      case '--test-http-idle-ms': options.httpIdleTimeoutMs = Number(arguments_[++i]); options.testIdleOverride = true; break;
  ...
  if (options.testIdleOverride && (!options.testMode || !Number.isInteger(options.httpIdleTimeoutMs)
      || options.httpIdleTimeoutMs < 100 || options.httpIdleTimeoutMs > HTTP_IDLE_TIMEOUT_MS)) throw new Error('invalid_test_timeout');
...
function boundConnect(request) {
  // A generous model-idle budget must not also lengthen TCP connection failure.
  request.once('socket', socket => {
    if (!socket.connecting) return;
    const timer = setTimeout(() => request.destroy(new Error('upstream_connect_timeout')), CONNECT_TIMEOUT_MS);
    timer.unref();
    const clear = () => { clearTimeout(timer); socket.off('connect', clear); request.off('close', clear); };
...
function main(options) {
  ...
  const state = (includeObservations = false) => ({ instanceId, protocolVersion: 1, transportVersion: 2,
    httpIdleTimeoutMs: options.httpIdleTimeoutMs, upstreamConnectTimeoutMs: CONNECT_TIMEOUT_MS,
    websocketHandshakeIdleTimeoutMs: WS_HANDSHAKE_TIMEOUT_MS, pid: process.pid, host: HOST, port: listenPort,
  ...
  const server = http.createServer({ maxHeaderSize: 32_768 }, (request, response) => {
    ...
    const upstream = http.request({ host: HOST, port: upstreamPort, method: request.method, path: route.target,
      headers: requestHeaders(request, upstreamPort, false), agent, timeout: options.httpIdleTimeoutMs });
    boundConnect(upstream);
    ...
    upstream.on('timeout', () => upstream.destroy(new Error('upstream_idle_timeout')));
    upstream.on('error', error => {
      const idle = error.message === 'upstream_idle_timeout';
      const status = idle ? 504 : 502;
      operation.observe('transport_error', status);
      httpError(response, status, idle ? 'upstream_idle_timeout' : 'upstream_unavailable');
    });
  ...
  server.on('upgrade', (request, socket, head) => {
    ...
    const upstream = http.request({ host: HOST, port: upstreamPort, method: 'GET', path: route.target,
      headers: requestHeaders(request, upstreamPort, true), agent: false, timeout: WS_HANDSHAKE_TIMEOUT_MS });
    boundConnect(upstream);
    operation.upstream = upstream;
    let upgraded = false;
    ...
    upstream.on('upgrade', (result, upstreamSocket, upstreamHead) => {
      ...
      upgraded = true;
      // The HTTP handshake budget does not apply to the upgraded stream.
